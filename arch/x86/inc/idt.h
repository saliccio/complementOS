...
typedef struct {
	u16_ct low_offset;  // Lower 16 bits for the handler's address
	u16_ct segment_selector;  // Segment selector which must point a valid entry in GDT. Bits 0-1: Requested privilege level. Bit 2: If 0, GDT is used, if 1, LDT is used. Bits 3-15: Index of the referenced GDT or LDT entry.
	...
	u8_ct flags;
	u16_ct high_offset;  // Higher 16 bits for the handler's address
...
typedef struct {
	...
	u32_ct base;  // Starting address of the IDT
