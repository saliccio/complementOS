...
void paging_init()
{
    ...
    for (u32_ct i = 0; i < 1024; i++)
    { // Apply identity paging for the first 4 MB
    ...
    __asm__("mov %0, %%cr3"
            :
            : "a"(page_directory));  // Move page directory address into control
                                     // register 3
...
/*void* allocate_pages(u32_ct page_table_index, u32_ct no_of_pages, u32_ct
physical_address) { if (page_table_index > 1022) return NULL;

    u32_ct* page_table_starting_address = (u32_ct*)
VIRTUAL_ADDRESS_OF_PAGE_TABLE(page_table_index); for (u32_ct i = 0; i <
no_of_pages; i++) { u32_ct* next_available_entry_ptr =
page_table_starting_address + next_available_indexes[page_table_index]; while
(*next)

        *next_available_entry_ptr = PAGE_TABLE_ENTRY(physical_address);
        next_available_indexes[page_table_index] =
(next_available_indexes[page_table_index] + 1) % PAGE_TABLE_SIZE;
    }
}*/